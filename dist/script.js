...
const determineOpacity = (currentTime, lastImpactTime, baseOpacity, maxOpacity, duration, angle) => {
  const isOnStartingPoint = angle % (2 * Math.PI) === 0;
  const isOnHalfCircle = angle % Math.PI === 0;

  if (isOnStartingPoint || isOnHalfCircle) {
    return maxOpacity;
  }
